...
- `npm run test:integration` — 39 tests passing across 8 files
...
| Phase 4 — Query/data-access/impersonation consistency | **Partial, not ready to close** | Centralized query modules exist, but consumers still use inline reads and the deprecated `@/lib/supabase/queries` shim. Admin-client soft-delete filtering and neighborhood scoping need a bounded migration and tests. |
...
The deprecated query shim still has consumers, including layout and notification code. Several application pages still use inline reads. The next implementation slice must migrate the known leak-prone paths first:

- posts page;
- member profile items;
- My Loans;
- item detail and active-loan reads;
- user-request reads;
- dashboard and directory reads where admin clients may bypass RLS.

Admin/service-role query tests must prove that soft-deleted posts, items, memberships, and loans remain hidden unless a documented history path requests them explicitly.
...
Migrate the highest-risk reads to the centralized query layer and close the protected-mutation inventory without starting the broader UI cleanup.

### Work sequence

1. Add or correct query functions for posts, items, memberships, loans, and neighborhood membership validation.
2. Migrate the posts page, member-profile items, My Loans, item detail, active-loan/user-request reads, and dashboard/admin-client reads.
3. Add admin-client query tests for soft-delete and neighborhood predicates.
4. Search all protected mutation call sites and classify each one as:
...
Stop when the known query leaks are migrated, the migrated scope has no deprecated shim imports, admin-client query tests pass, and every remaining protected write is explicitly classified. Keep the unit, integration, typecheck, and lint commands green.
