...
Standard CI runs lint, typecheck, unit tests, and build on every push and pull request. For a PR intended for release, run the manually dispatched **Full Validation** workflow against the final branch or ref before merging:

1. Push the final commits to the branch.
2. Open **Actions → Full Validation → Run workflow**.
3. Enter the branch or ref containing those commits.
4. Wait for integration tests and all E2E shards to pass.
5. Do not merge or release until Full Validation is green.

Full Validation runs integration tests against a fresh local Supabase instance and E2E tests against the isolated E2E Supabase project. It does not modify production or advance the `prod` branch. The production release workflow retains its own final E2E gate.
